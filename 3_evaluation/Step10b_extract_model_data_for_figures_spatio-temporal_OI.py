...
Extract 2D model data within the configuration domain and 
linearly interpolate model fields to the time of the OI map. 

(for CMEMS 3D data, we use data at the upper layer). 

Save fields: SSH, U, V, speed and Ro.

Fields only used in *Step11b* to make figures.

Note: In *Step10c* we extract data in a bigger domain than the configuration
domain. In this way, we can then interpolate model data onto the
reconstruction grid without extrapolation or having nans due to the lack
of data near the configuration periphery (due to the different resolution
between model and reconstruction grids).
...
if __name__ == '__main__':        
    ...
    '''
        Open pseudo-observation to limit  
    ...
    for file in oi_files: 
        ...
        ''' 1) Read time of the interpolated fields '''
        ...
        # dep_ctd   = ncpo.variables['dep_ctd'][:]  
        lat_ctd   = ncpo.variables['lat_ctd'][:]   
        lon_ctd   = ncpo.variables['lon_ctd'][:]  
        # tem_ctd   = ncpo.variables['tem_ctd'][:]      
        # sal_ctd   = ncpo.variables['sal_ctd'][:]  
        ncpo.close() 
        ...
        ''' 
           Only use model data inside the sampling domain and time  
           defined by lon_ctd, lat_ctd, time_ctd,
           different for each configuration.
           
           # extracting model data for the sampling period +- 1 day
        '''
        
        lonm_reg, latm_reg, timem_reg, \
               um_reg, vm_reg, ssh_reg = to.limit_exact_2D_model_data(\
                                         u2d_all, v2d_all, 
    ...
    f_model = open(dir_dic + region + '_' + model + '_ocean_truth_SSH_speed_Ro.pkl','wb')
